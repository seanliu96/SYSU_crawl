...
import SYSU.fetch_free_proxies as fetch_free_proxies
...
class HttpProxyMiddleware(object):
    ...
    def fetch_new_proxies(self):
        ...
        new_proxies = fetch_free_proxies.fetch_all()
        ...
        for np in new_proxies:
            if self.url_in_proxies("http://" + np):
                ...
                self.proxies.append({"proxy": "http://"  + np,
                                     "valid": True,
                                     "count": 0})
    ...
    def process_request(self, request, spider):
        ...
        if self.proxy_index > 0  and datetime.now() > (self.last_no_proxy_time + timedelta(minutes=self.recover_interval)):
            ...
            self.last_no_proxy_time = datetime.now()
            self.proxy_index = 0
        request.meta["dont_redirect"] = False  # 有些代理会把请求重定向到一个莫名其妙的地址
